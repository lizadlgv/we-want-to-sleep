...
public class MathFunctionTest {
    ...
    public void testAndThen() {
        assertEquals(composite.apply(4), 2);
        assertEquals(division.andThen(sqr).apply(16), 64);
        assertEquals(root.andThen(constant).apply(7), 3);
        assertEquals(unit.andThen(division).andThen(identity).apply(67), 0.5);
    }
}